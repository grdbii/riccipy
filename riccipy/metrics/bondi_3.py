...
r, u, th, ph = coords
Q, U, V, gamma = functions
metric = zeros(4)
metric[1, 1] = (
    -exp(2 * Q(r, u, th) + 2 * gamma(r, u, th))
    * (
        V(r, u, th) * exp(-2 * gamma(r, u, th))
        - exp(-2 * Q(r, u, th)) * U(r, u, th) ** 2 * r ** 3
    )
    / r
)
metric[2, 2] = r ** 2 * exp(2 * gamma(r, u, th))
metric[3, 3] = r ** 2 * exp(-2 * gamma(r, u, th)) * sin(th) ** 2
metric[0, 1] = metric[1, 0] = -exp(2 * Q(r, u, th))
metric[1, 2] = metric[2, 1] = -U(r, u, th) * r ** 2 * exp(2 * gamma(r, u, th))
